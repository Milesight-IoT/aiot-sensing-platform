...
public class BackwardCompatibilityAdaptor implements MqttTransportAdaptor {
    ...
    @Override
    public Optional<MqttMessage> convertToPublish(MqttDeviceAwareSessionContext ctx, TransportProtos.ToDeviceMsMsg msMsg, String topicBase)
            throws UnsupportedEncodingException {
        try {
            return protoAdaptor.convertToPublish(ctx, msMsg, topicBase);
        } catch (Exception e) {
            log.trace("[{}] failed to process post attributes request msg: {} due to: ", ctx.getSessionId(), msMsg, e);
            return jsonAdaptor.convertToPublish(ctx, msMsg, topicBase);
        }
    }
    ...
    public TransportProtos.ToServerRpcMsMsg convertToServerRpcMsMsg(MqttDeviceAwareSessionContext ctx, MqttPublishMessage mqttMsg, String topicBase) throws AdaptorException {
        try {
            return protoAdaptor.convertToServerRpcMsMsg(ctx, mqttMsg, topicBase);
        } catch (AdaptorException e) {
            log.trace("[{}] failed to process to server rpc request msg: {} due to: ", ctx.getSessionId(), mqttMsg, e);
            return jsonAdaptor.convertToServerRpcMsMsg(ctx, mqttMsg, topicBase);
        }
    }
