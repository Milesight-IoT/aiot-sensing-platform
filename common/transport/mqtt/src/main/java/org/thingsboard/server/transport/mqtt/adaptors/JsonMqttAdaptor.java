...
public class JsonMqttAdaptor implements MqttTransportAdaptor {
    ...
    @Override
    public Optional<MqttMessage> convertToPublish(MqttDeviceAwareSessionContext ctx, TransportProtos.ToDeviceMsMsg toDeviceMsMsg, String topicBase) {
        String requestKey = toDeviceMsMsg.getRequestKey();
        if (StringUtils.isNotBlank(requestKey)) {
            String message = toDeviceMsMsg.getMessage();
            MqttPublishMessage mqttPublishMsg = null;
            try {
    ...
    @Override
    public TransportProtos.ToServerRpcMsMsg convertToServerRpcMsMsg(MqttDeviceAwareSessionContext ctx, MqttPublishMessage inbound, String topicBase) throws AdaptorException {
        ...
        try {
            ...
            return JsonConverter.convertToServerRpcMsMsg(new JsonParser().parse(payload), requestKey);
