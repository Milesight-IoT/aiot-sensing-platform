...
public class MqttTransportHandler extends ChannelInboundHandlerAdapter implements GenericFutureListener<Future<? super Void>>, SessionMsgListener {
    ...
    private TopicType msSubTopicType;
    ...
    public void channelRead(ChannelHandlerContext ctx, Object msg) {
        ...
        try {
            if (msg instanceof MqttMessage) {
                ...
                log.error("[{}] Received non mqtt message: {}", sessionId, msg.getClass().getSimpleName());
                ctx.close();
            }
        } finally {
    ...
    // 消息类型为连接时转入processConnect, 设备session为临时的转入processProvisionSessionMsg, 否则转入enqueueRegularSessionMsg方法
    ...
    // 根据消息的类型进行转发, 比如: 发布/订阅/取消订阅/取消连接等等
        ...
        switch (msg.fixedHeader().messageType()) {
            ...
            case DISCONNECT:
                if (deviceSessionCtx.getDeviceInfo() != null) {
                    log.info("DISCONNECT1, deviceName:{}, deviceId:{}", deviceSessionCtx.getDeviceInfo().getDeviceName(), deviceSessionCtx.getDeviceInfo().getDeviceId());
                } else {
                    log.info("DISCONNECT2");
                }
                ctx.close();
                break;
            case PUBACK:
    ...
    private void processDevicePublish(ChannelHandlerContext ctx, MqttPublishMessage mqttMsg, String topicName, int msgId) {
        try {
            ...
            if (topicName.startsWith(MqttTopics.BASIC_DEVICES_MS_TOPIC)) {
                // MS消息发布
                TransportProtos.ToServerRpcMsMsg toServerRpcMsMsg = payloadAdaptor.convertToServerRpcMsMsg(deviceSessionCtx, mqttMsg, MqttTopics.BASIC_DEVICES_MS_TOPIC);
                transportService.process(deviceSessionCtx.getSessionInfo(), toServerRpcMsMsg, getPubAckCallback(ctx, msgId, toServerRpcMsMsg));
...
//            ctx.close();
        ...
        for (MqttTopicSubscription subscription : mqttMsg.payload().topicSubscriptions()) {
            ...
            try {
                if (sparkplugSessionHandler != null) {
                    ...
                    switch (topic) {
                        case MqttTopics.DEVICES_MS:
                            // 活跃状态
                            processMsSubscribe(grantedQoSList, topic, reqQoS, TopicType.V1);
                            activityReported = true;
                            break;
     ...
     * 二开TOPIC订阅
     */
    private void processMsSubscribe(List<Integer> grantedQoSList, String topic, MqttQoS reqQoS, TopicType topicType) {
        TransportProtos.SubscribeToDeviceMsMsg build = TransportProtos.SubscribeToDeviceMsMsg.newBuilder().build();
        transportService.process(deviceSessionCtx.getSessionInfo(), build, null);
        msSubTopicType = topicType;
        registerSubQoS(topic, grantedQoSList, reqQoS);
    }
                    ...
                    switch (topicName) {
                        case MqttTopics.DEVICES_MS:
                            transportService.process(deviceSessionCtx.getSessionInfo(),
                                    TransportProtos.SubscribeToDeviceMsMsg.newBuilder().setUnsubscribe(true).build(), null);
                            activityReported = true;
                            break;
                        case MqttTopics.DEVICE_ATTRIBUTES_TOPIC:
                        case MqttTopics.DEVICE_ATTRIBUTES_SHORT_TOPIC:
    ...
    private void processAuthTokenConnect(ChannelHandlerContext ctx, MqttConnectMessage connectMessage) {
        ...
        log.info("CONNECTION, userName:{},clientId:{}", userName, connectMessage.payload().clientIdentifier());
    ...
    private void onValidateDeviceResponse(ValidateDeviceCredentialsResponse msg, ChannelHandlerContext ctx, MqttConnectMessage connectMessage) {
        if (!msg.hasDeviceInfo()) {
            ...
            log.info("CONNECTION_REFUSED_NOT_AUTHORIZED");
            ctx.close();
        } else {
            String deviceName = msg.getDeviceInfo().getDeviceName();
            ...
            transportService.process(deviceSessionCtx.getSessionInfo(), SESSION_EVENT_MSG_OPEN, new TransportServiceCallback<Void>() {
                ...
                public void onSuccess(Void msg) {
                    ...
                    log.info("CONNECTION_ACCEPTED, deviceName:{}, deviceId:{}", deviceName, deviceId);
                ...
                public void onError(Throwable e) {
                    if (e instanceof TbRateLimitsException) {
                        ...
                        log.info("TbRateLimitsException Failed");
                    } else {
    ...
    @Override
    public void onToDeviceMs(UUID sessionId, TransportProtos.ToDeviceMsMsg toDeviceMsMsg) {
        log.trace("[{}] Received get attributes response", sessionId);
        String topicBase = msSubTopicType.getMsTopicBase();
        MqttTransportAdaptor adaptor = deviceSessionCtx.getAdaptor(msSubTopicType);
        try {
            adaptor.convertToPublish(deviceSessionCtx, toDeviceMsMsg, topicBase).ifPresent(deviceSessionCtx.getChannel()::writeAndFlush);
        } catch (Exception e) {
            log.trace("[{}] Failed to convert device ms to MQTT msg", sessionId, e);
        }
    }
