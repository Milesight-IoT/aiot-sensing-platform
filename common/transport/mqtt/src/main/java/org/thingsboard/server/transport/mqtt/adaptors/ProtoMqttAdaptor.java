...
public class ProtoMqttAdaptor implements MqttTransportAdaptor {
    ...
    @Override
    public Optional<MqttMessage> convertToPublish(MqttDeviceAwareSessionContext ctx, TransportProtos.ToDeviceMsMsg toDeviceMsMsg, String topicBase) throws UnsupportedEncodingException {
        String requestKey = toDeviceMsMsg.getRequestKey();
        if (StringUtils.isNotBlank(requestKey)) {
            String message = toDeviceMsMsg.getMessage();
            MqttPublishMessage mqttPublishMsg;
            try {
                mqttPublishMsg = createMqttPublishMsg(ctx, topicBase + requestKey, message.getBytes("UTF-8"));
            } catch (UnsupportedEncodingException e) {
                log.warn("Failed to decode ms request", e);
                throw new RuntimeException(e);
            }
            return Optional.of(mqttPublishMsg);
        }
        return Optional.empty();
    }

    @Override
    public TransportProtos.ToServerRpcMsMsg convertToServerRpcMsMsg(MqttDeviceAwareSessionContext ctx, MqttPublishMessage mqttMsg, String topicBase) throws AdaptorException {
        byte[] bytes = toBytes(mqttMsg.payload());
        String topicName = mqttMsg.variableHeader().topicName();
        try {
            String requestKey = getRequestKey(topicName, topicBase);
            return ProtoConverter.convertToServerRpcMsMsg(bytes, requestKey);
    ...
    private String getRequestKey(String topicName, String topic) {
        return topicName.substring(topic.length());
    }
}
