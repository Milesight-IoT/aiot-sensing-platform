...
public class SystemUtil {
    ...
    public static Optional<Long> getCassandraSpaceAvailable(String path) {
        try {
            FileStore store = Files.getFileStore(Paths.get(path));
            return Optional.of(store.getUsableSpace());
        } catch (Exception e) {
            log.error("Failed to get cassandra space:", e);
        }
        return Optional.empty();
    }

    public static Optional<Long> getCassandraSpaceTotal(String path) {
        try {
            FileStore store = Files.getFileStore(Paths.get(path));
    ...
    public static Optional<Integer> getCpuUsage() {
        try {
            return Optional.of((int) (HARDWARE.getProcessor().getSystemCpuLoad(1000L) * 100.0));
        } catch (Exception e) {
