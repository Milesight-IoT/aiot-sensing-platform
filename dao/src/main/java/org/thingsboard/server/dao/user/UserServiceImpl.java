...
public class UserServiceImpl extends AbstractEntityService implements UserService {
    ...
    private void removeUserCache(TenantId tenantId, UserId userId) {
        User user = userDao.findById(tenantId, userId.getId());
        if (user != null) {
            // 去除自定义缓存
            loginCache.remove(user.getName());
        }
    }
    ...
    public UserCredentials saveUserCredentials(TenantId tenantId, UserCredentials userCredentials) {
        ...
        removeUserCache(tenantId, userCredentials.getUserId());
    ...
    public void deleteUser(TenantId tenantId, UserId userId) {
        ...
        // 删除账号缓存
        removeUserCache(tenantId, userId);
