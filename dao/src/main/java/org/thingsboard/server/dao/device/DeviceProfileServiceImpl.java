...
// //                () -> deviceProfileDao.findByName(tenantId, profileName), true);
    ...
    public DeviceProfile updateDeviceProfileOtaId(UUID deviceProfileId, OtaPackageType type, UUID otaPackageId) {
        return deviceProfileDao.updateDeviceProfileOtaId(deviceProfileId, type, otaPackageId);
    }

    @Override
    public List<DeviceProfile> findByOtaPackageId(UUID otaPackageId, OtaPackageType type) {
        if (type.equals(OtaPackageType.FIRMWARE)) {
            return deviceProfileDao.findByFirmwareId(otaPackageId);
        }
        if (type.equals(OtaPackageType.SOFTWARE)) {
            return deviceProfileDao.findBySoftwareId(otaPackageId);
        }
        if (type.equals(OtaPackageType.CONFIGURE)) {
            return deviceProfileDao.findByConfigureId(otaPackageId);
        }
    ...
    private PaginatedRemover<TenantId, DeviceProfile> tenantDeviceProfilesRemover =
            new PaginatedRemover<>() {
                ...
                protected PageData<DeviceProfile> findEntities(TenantId tenantId, TenantId id, PageLink pageLink) {
                    return deviceProfileDao.findDeleteDeviceProfiles(id, pageLink, ProfileTypeEnum.CUSTOM_MODEL.getValue());
                }
