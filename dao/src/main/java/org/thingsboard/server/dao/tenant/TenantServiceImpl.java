...
public class TenantServiceImpl extends AbstractCachedEntityService<TenantId, Tenant, TenantEvictEvent> implements TenantService {
    ...
    public Tenant saveTenant(Tenant tenant) {
        ...
        if (tenant.getId() == null) {
            // 2023-04-07 注释原因：初始化数据不需要default
//            deviceProfileService.createDefaultDeviceProfile(savedTenant.getId());
    ...
    @Override
    public Tenant findBySearchText(String text) {
        log.trace("Executing findBySearchText");
        return tenantDao.findTenantBySearchText(text);
    }
