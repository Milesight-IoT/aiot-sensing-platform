...
public class DefaultEntityQueryRepository implements EntityQueryRepository {
    ...
    public long countEntitiesByQuery(TenantId tenantId, CustomerId customerId, EntityCountQuery query) {
        ...
        if (query.getKeyFilters() == null || query.getKeyFilters().isEmpty()) {
            ...
            result = inActiveNum(ctx, countQuery, result);

            return result;
        }
    }

    private Long inActiveNum(QueryContext ctx, String countQuery, Long result) {
        try {
            String ss = "select count(id) from (select entities.* from (select e.id id, 'DEVICE' entity_type from device e "
                    + "where e.tenant_id=:permissions_tenant_id) entities join LATERAL (select * from attribute_kv alias2"
                    + " WHERE alias2.entity_id=entities.id AND alias2.entity_type='DEVICE' AND alias2.attribute_key=:alias2_key_id  "
                    + "AND ((alias2.bool_v is not null and alias2.bool_v = :alias2_bool_v_1)) ORDER BY alias2.last_update_ts DESC limit 1) "
                    + "as alias2 ON true ) result ";
            Boolean value = (Boolean) ctx.getValue("alias2_bool_v_1");
            if (countQuery.equals(ss) && Boolean.FALSE.equals(value)) {
                String exitsSql = "SELECT count(*) FROM device a "
                        + " LEFT JOIN attribute_kv b ON b.entity_id = a.id "
                        + " and b.entity_type = 'DEVICE' AND b.attribute_key = :alias2_key_id"
                        + " WHERE b.entity_id IS NULL AND a.tenant_id = :permissions_tenant_id";

                Long exitsNullNum = jdbcTemplate.queryForObject(exitsSql, ctx, Long.class);
                result = result + (exitsNullNum == null ? 0 : exitsNullNum);
            }
        } catch (Exception e) {
            log.error("inActiveNum ctx error: ", e);
        }
        return result;
    }
