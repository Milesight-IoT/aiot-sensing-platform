...
public class BaseOtaPackageService extends AbstractCachedEntityService<OtaPackageCacheKey, OtaPackageInfo, OtaPackageCacheEvictEvent> implements OtaPackageService {
    ...
    public String generateChecksum(ChecksumAlgorithm checksumAlgorithm, ByteBuffer data) {
        ...
        if (ChecksumAlgorithm.CRC32.equals(checksumAlgorithm)) {
            return getCrc32Str(data);
        } else {
    ...
    @NotNull
    private static String getCrc32Str(ByteBuffer data) {
...
//                () -> otaPackageInfoDao.findById(tenantId, otaPackageId.getId()), true);
    ...
    public OtaPackageInfo findFirmwareByDeviceId(UUID deviceId) {
        return otaPackageInfoDao.findFirmwareByDeviceId(deviceId);
    }

    @Override
    public OtaPackageInfo findFirmwareByCredentialsId(String credentialsId) {
        return otaPackageInfoDao.findFirmwareByCredentialsId(credentialsId);
    }

    @Override
    public OtaPackageInfo findConfigureByDeviceId(UUID deviceId) {
        return otaPackageInfoDao.findConfigureByDeviceId(deviceId);
    }

    @Override
    public OtaPackageInfo findConfigureByCredentialsId(String credentialsId) {
        return otaPackageInfoDao.findConfigureByCredentialsId(credentialsId);
    }

    @Override
    public OtaPackageInfo findByTenantIdAndTitleAndVersion(TenantId tenantId, String title, String version) {
        return otaPackageInfoDao.findByTenantIdAndTitleAndVersion(tenantId.getId(), title, version);
    }

    @Override
    public OtaPackageInfo findByTenantIdAndTitle(TenantId tenantId, String title) {
        return otaPackageInfoDao.findByTenantIdAndTitle(tenantId.getId(), title);
    }
