...
public interface DeviceRepository extends JpaRepository<DeviceEntity, UUID>, ExportableEntityRepository<DeviceEntity> {
    ...
    @Query(value = "select d.* from device_credentials dc left join device d on dc.device_id = d.id " +
            "where dc.credentials_id = :credentialsId and d.tenant_id = :tenantId", nativeQuery = true)
    DeviceEntity findByCredentialsId(@Param("tenantId") UUID tenantId, @Param("credentialsId") String credentialsId);
}
