...
public class DashboardServiceImpl extends AbstractEntityService implements DashboardService {
    ...
    public Dashboard findByCreatedTime(UUID tenantId, long time) {
        return dashboardDao.findByTenantIdAndCreatedTime(tenantId, time);
    }

    @Override
    public boolean hasByTenantIdAndIdOrName(TenantId tenantId, String dashboardId, String dashboardName) {
        List<Dashboard> byTenantIdAndTitle = dashboardDao.findByTenantIdAndTitle(tenantId.getId(), dashboardName);
        if (StringUtils.isBlank(dashboardId)) {
            return !byTenantIdAndTitle.isEmpty();
        }
        return byTenantIdAndTitle.stream().anyMatch(dashboard -> {
            boolean idSame = dashboard.getId().getId().toString().equals(dashboardId);
            boolean nameSame = dashboard.getName().equals(dashboardName);
            // id不相同且名称相同说明名称已存在不是更新操作
