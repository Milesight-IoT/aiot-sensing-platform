...
public class DeviceServiceImpl extends AbstractCachedEntityService<DeviceCacheKey, Device, DeviceCacheEvictEvent> implements DeviceService {
    ...
    private final ConcurrentMap<DeviceCacheKey, Integer> retryMap = new ConcurrentHashMap<>();
    ...
    public Device findDeviceById(TenantId tenantId, DeviceId deviceId) {
        ...
        validateId(deviceId, INCORRECT_DEVICE_ID + deviceId);
        DeviceCacheKey key = new DeviceCacheKey(tenantId, deviceId);
        Device transaction = cache.getAndPutInTransaction(key,
                () -> {
        ...
        retryToGetDeviceInformation(tenantId, deviceId, key, transaction);
        return transaction;
    }

    private void retryToGetDeviceInformation(TenantId tenantId, DeviceId deviceId, DeviceCacheKey key, Device transaction) {
        if (transaction == null) {
            Integer retry = retryMap.putIfAbsent(key, 0);
            if (retry == null || retry >= RETRY_TIMES) {
                // 驱逐重试
                DeviceCacheEvictEvent deviceCacheEvictEvent = new DeviceCacheEvictEvent(tenantId, deviceId, null, null);
    ...
    public Device save(Device device) {
        return deviceDao.save(null, device);
    }
    ...
    private Device doSaveDevice(Device device, String accessToken, boolean doValidate) {
        ...
        if (device.getId() == null) {
            ...
            DeviceCredentials deviceCredentials = deviceCredentialsService.findDeviceCredentialsByDeviceId(savedDevice.getTenantId(), device.getId());
    ...
    private Device saveDeviceWithoutCredentials(Device device, boolean doValidate) {
        ...
        DeviceCacheEvictEvent sysEvictEvent  = new DeviceCacheEvictEvent(TenantId.SYS_TENANT_ID, device.getId(), device.getName(), oldDevice != null ? oldDevice.getName() : null);
        try {
            ...
            publishEvictEvent(deviceCacheEvictEvent);
            publishEvictEvent(sysEvictEvent);
            if (device.getId() == null) {
            ...
            handleEvictEvent(sysEvictEvent);
    ...
    public void deleteDevice(final TenantId tenantId, final DeviceId deviceId) {
        ...
        // 删除设备能力
        deviceAbilityService.deleteByDeviceId(deviceId.getId());
    ...
    @Override
    public CurrentDeviceOtaPackage getDeviceOtaPackageById(TenantId tenantId, DeviceId deviceId) throws ExecutionException, InterruptedException {
        List<AttributeKvEntry> clientScope = attributesService.find(tenantId, deviceId, DataConstants.CLIENT_SCOPE, TelemetryConstants.CURRENT_DEVICE_OTA_PACKAGE).get();
        CurrentDeviceOtaPackage.CurrentDeviceOtaPackageBuilder builder = CurrentDeviceOtaPackage.builder();
        for (AttributeKvEntry attributeKvEntry : clientScope) {
            if (attributeKvEntry.getKey().equals(TelemetryConstants.CURRENT_FW_TITLE)) {
                builder.fwTitle(attributeKvEntry.getValueAsString());
            }
            if (attributeKvEntry.getKey().equals(TelemetryConstants.CURRENT_FW_CHECKSUM)) {
                builder.fwChecksum(attributeKvEntry.getValueAsString());
            }
            if (attributeKvEntry.getKey().equals(TelemetryConstants.CURRENT_CF_TITLE)) {
                builder.cfTitle(attributeKvEntry.getValueAsString());
            }
            if (attributeKvEntry.getKey().equals(TelemetryConstants.CURRENT_CF_CHECKSUM)) {
    ...
    @Override
    public Device findByTenantIdAndName(TenantId tenantId, String name) {
        return deviceDao.findByTenantIdAndName(tenantId.getId(), name);
    }

    @Override
    public Device findByCredentialsId(TenantId tenantId, String credentialsId) {
        return deviceDao.findByCredentialsId(tenantId.getId(), credentialsId);
    }
