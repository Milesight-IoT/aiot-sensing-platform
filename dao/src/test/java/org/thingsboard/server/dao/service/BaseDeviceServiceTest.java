...
public abstract class BaseDeviceServiceTest extends AbstractServiceTest {
    ...
    public void testFindDevicesByTenantIdAndName() {
        ...
        for (int i = 0; i < 143; i++) {
            ...
            device.setName(name);
            device.setType("default");
            devicesTitle1.add(new DeviceInfo(deviceService.saveDevice(device), null, false, "default", "credentialsId", false));
        }
        ...
        for (int i = 0; i < 175; i++) {
            ...
            device.setName(name);
            device.setType("default");
            devicesTitle2.add(new DeviceInfo(deviceService.saveDevice(device), null, false, "default", "credentialsId", false));
        }
    ...
    public void testFindDevicesByTenantIdAndCustomerId() {
        ...
        for (int i = 0; i < 278; i++) {
            ...
            devices.add(new DeviceInfo(deviceService.assignDeviceToCustomer(tenantId, device.getId(), customerId), customer.getTitle(), customer.isPublic(), "default", "credentialsId", false));
        }
