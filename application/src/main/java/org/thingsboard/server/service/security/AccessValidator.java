...
public class AccessValidator {
    ...
    public void validate(SecurityUser currentUser, Operation operation, EntityId entityId, FutureCallback<ValidationResult> callback) {
        switch (entityId.getEntityType()) {
            ...
            case DASHBOARD_RULE_DEVICES:
                validateDashboardRuleDevices(currentUser, operation, entityId, callback);
                return;
            default:
    ...
    /**
     * 增加AccessValidator,否则会点击属性会显示Not Implemented
     * @param currentUser
     * @param operation
     * @param entityId
     * @param callback
     */
    private void validateDashboardRuleDevices(final SecurityUser currentUser, Operation operation, EntityId entityId, FutureCallback<ValidationResult> callback) {
        if (currentUser.isSystemAdmin()) {
            callback.onSuccess(ValidationResult.accessDenied(SYSTEM_ADMINISTRATOR_IS_NOT_ALLOWED_TO_PERFORM_THIS_OPERATION));
        } else {
            ListenableFuture<DashboardRuleDevices> DashboardRuleDevicesFuture =
                    dashboardRuleDevicesService.findDashboardRuleDevicesByIdAsync(currentUser.getTenantId(), new DashboardRuleDevicesId(entityId.getId()));
            Futures.addCallback(DashboardRuleDevicesFuture, getCallback(callback, customer -> {
                if (customer == null) {
                    return ValidationResult.entityNotFound("Customer with requested id wasn't found!");
                } else {
                    try {
                        accessControlService.checkPermission(currentUser, Resource.DASHBOARD_RULE_DEVICES, operation, entityId, customer);
                    } catch (ThingsboardException e) {
                        return ValidationResult.accessDenied(e.getMessage());
                    }
                    return ValidationResult.ok(customer);
