...
public class DefaultDeviceStateService extends AbstractPartitionBasedService<DeviceId> implements DeviceStateService {
    ...
    public void onDeviceDisconnect(TenantId tenantId, DeviceId deviceId) {
        ...
        long ts = System.currentTimeMillis();
        // MS 修正
        redressLastInactivityAlarmTime(ts, deviceId, stateData);
    ...
    void checkAndUpdateState(@Nonnull DeviceId deviceId, @Nonnull DeviceStateData state) {
        ...
        if (state.getState().isActive()) {
            redressLastInactivityAlarmTime(ts, deviceId, state);
            updateInactivityStateIfExpired(ts, deviceId, state);
        } else {
            // trying to fix activity state
            if (isActive(ts, state.getState())) {
    ...
    /**
     * 修正INACTIVITY_ALARM_TIME时间
     *
     * @param ts              时间戳
     * @param deviceId        设备id
     * @param deviceStateData 设备状态
     */
    private void redressLastInactivityAlarmTime(long ts, DeviceId deviceId, DeviceStateData deviceStateData) {
        DeviceState dataState = deviceStateData.getState();
        boolean active = dataState.isActive();
        if (!active) {
            log.debug("The device is not online and does not need to be corrected,deviceId={}", deviceId);
            return;
        }
        long lastInactivityAlarmTime = dataState.getLastInactivityAlarmTime();
        long lastActivityTime = dataState.getLastActivityTime();
        if (lastInactivityAlarmTime != 0L && lastActivityTime != 0L && (lastInactivityAlarmTime >= lastActivityTime || lastInactivityAlarmTime > ts)) {
            // 修正INACTIVITY_ALARM_TIME时间
            save(deviceId, INACTIVITY_ALARM_TIME, lastActivityTime - 1);
            dataState.setLastInactivityAlarmTime(lastActivityTime - 1);
        }
    }
    ...
    private List<DeviceStateData> fetchDeviceStateDataUsingSeparateRequests(List<DeviceIdInfo> deviceIds) {
        List<DeviceId> deviceIdList = deviceIds.stream().map(DeviceIdInfo::getDeviceId).collect(Collectors.toList());
        List<Device> devices = deviceService.findDevicesByIds(deviceIdList);
