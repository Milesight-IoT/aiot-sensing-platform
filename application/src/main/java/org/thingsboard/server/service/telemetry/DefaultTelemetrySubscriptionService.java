...
public class DefaultTelemetrySubscriptionService extends AbstractSubscriptionService implements TelemetrySubscriptionService {
    ...
    // application模块添加遥测数据入口
    @Override
    public void saveAndNotifyMs(TenantId tenantId, TbMsg msg, List<TsKvEntry> ts, long ttl, FutureCallback<Void> callback) {
        doSaveAndNotifyMs(tenantId, msg, ts, ttl, callback, true);
    }

    public void saveAndNotifyInternalMs(TenantId tenantId, TbMsg msg, List<TsKvEntry> ts, long ttl, FutureCallback<Integer> callback) {
        ListenableFuture<Integer> saveFuture = tsService.saveMs(tenantId, msg, ts, ttl);
        addCallbacks(tenantId, msg.getOriginator(), ts, callback, saveFuture);
    }

    @Override
    public void saveWithoutLatestAndNotifyMs(TenantId tenantId, TbMsg msg, List<TsKvEntry> ts, long ttl, FutureCallback<Void> callback) {
        doSaveAndNotifyMs(tenantId, msg, ts, ttl, callback, false);
    }

    private void saveWithoutLatestAndNotifyInternalMs(TenantId tenantId, TbMsg msg, List<TsKvEntry> ts, long ttl, FutureCallback<Integer> callback) {
        ListenableFuture<Integer> saveFuture = tsService.saveWithoutLatestMs(tenantId, msg, ts, ttl);
        // 添加时序数据保存后回调方法
        addCallbacks(tenantId, msg.getOriginator(), ts, callback, saveFuture);
    }

    private void doSaveAndNotifyMs(TenantId tenantId, TbMsg msg, List<TsKvEntry> ts, long ttl, FutureCallback<Void> callback, boolean saveLatest) {
        checkInternalEntity(msg.getOriginator());
        boolean sysTenant = TenantId.SYS_TENANT_ID.equals(tenantId) || tenantId == null;
        if (sysTenant || apiUsageStateService.getApiUsageState(tenantId).isDbStorageEnabled()) {
            KvUtils.validate(ts);
            if (saveLatest) {
                saveAndNotifyInternalMs(tenantId, msg, ts, ttl, getCallback(tenantId, msg.getCustomerId(), sysTenant, callback));
            } else {
                saveWithoutLatestAndNotifyInternalMs(tenantId, msg, ts, ttl, getCallback(tenantId, msg.getCustomerId(), sysTenant, callback));
            }
        } else {
            callback.onFailure(new RuntimeException("DB storage writes are disabled due to API limits!"));
        }
