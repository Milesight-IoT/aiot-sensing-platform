...
public class DefaultTbDeviceService extends AbstractTbEntityService implements TbDeviceService {
    ...
    private final DeviceAbilityService deviceAbilityService;
    ...
    public Device save(Device device, Device oldDevice, String accessToken, User user) throws Exception {
        ...
        try {
            ...
            if (oldDevice == null) {
                DeviceAbility deviceAbility = DeviceAbility.builder()
                        .deviceId(savedDevice.getId().getId())
                        .ability(TelemetryConstants.IMAGE)
                        .abilityType((short) 2)
                        .build();
                deviceAbilityService.saveDeviceAbilityAndCache(deviceAbility);
            }
            autoCommit(user, savedDevice.getId());

            // 这里会导致设备被缓存,出现设备缓存为null,更新不了设备属性,先去掉
