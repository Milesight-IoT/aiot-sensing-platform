    ...
    public void handleTextMessage(WebSocketSession session, TextMessage message) {
        try {
            ...
            if (sessionMd != null) {
                ...
                log.trace("[{}][{}] Processing {}", sessionMd.sessionRef.getSecurityCtx().getTenantId(), session.getId(), payLoad);
                // if (payLoad.contains(DASHBOARD_ALARM_TABLE) || payLoad.contains(DASHBOARD_SNAPSHOT_PREVIEW)) {
                //     dbWebSocketService.handleWebSocketMsg(session, message, firstConnectTime);
                // } else {
                webSocketService.handleWebSocketMsg(sessionMd.sessionRef, payLoad);
                // }
            } else {
