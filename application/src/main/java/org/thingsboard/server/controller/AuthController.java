...
public class AuthController extends BaseController {
    ...
    public ObjectNode changePassword(
        ...
        String newPassword = StringUtils.isBlank(changePasswordRequest.getNewPassword()) ? "password" : changePasswordRequest.getNewPassword();
        SecurityUser securityUser = getCurrentUser();
    ...
    @ApiOperation(value = "判断当前密码是否存在")
    @PreAuthorize("hasAnyAuthority('SYS_ADMIN', 'TENANT_ADMIN', 'CUSTOMER_USER')")
    @RequestMapping(value = "/auth/existsPassword", method = RequestMethod.GET)
    @ResponseStatus(value = HttpStatus.OK)
    public Boolean existsPassword(
            @ApiParam("密码")
            @RequestParam(value = "password", required = true) String currentPassword) throws ThingsboardException {
        try {
            SecurityUser securityUser = getCurrentUser();
            UserCredentials userCredentials = userService.findUserCredentialsByUserId(TenantId.SYS_TENANT_ID, securityUser.getId());

            return passwordEncoder.matches(currentPassword, userCredentials.getPassword());
        } catch (Exception e) {
