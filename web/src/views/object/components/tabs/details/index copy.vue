...
async function searchDeviceList(isNode) {
    const queryParams = {
        pageSize: 2000,
        page: 0,
    ...
    while (hasNextPage) {
        const { data, hasNext } = await getUserDeviceList(queryParams);
        queryParams.page += 1;
...
const getCasOpts = async () => {
    if (!deviceNodes.length) deviceNodes = await searchDeviceList(true);
    const channels = formData.value.channels;
    let noProgress = false;
    for (let i = 0; i < channels.length; i++) {
        casProps.value.lazy = false;
        const id = channels[i][0];
        const index = deviceNodes.findIndex(item => item.value == id);
        if (index >= 0) {
    ...
    if (!channels.length) casOpts.value = deviceNodes;
