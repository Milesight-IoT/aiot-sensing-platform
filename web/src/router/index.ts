...
/** 公开连接 */
const publicList = ['/dashboards/'];
router.beforeEach(async (to: toRouteType, _from, next) => {
    console.log(to)
    const publicId = to.query?.publicId;
    ...
    function toCorrectRoute() {
        console.log(whiteList.includes(to.fullPath))
    ...
    if (userInfo || publicId) {
        ...
        if (_from?.name) {
            ...
            if (publicId && to.matched.length > 1) {
                const { token } = await useUserStoreHook().loginPublic(publicId)
        ...
        if (to.path !== '/login') {
            if (whiteList.indexOf(to.path) !== -1) {
                ...
                const Title = getConfig().Title;
                if (Title) document.title = `${transformI18n('menus.login')} | ${Title}`;
                else document.title = transformI18n('menus.login');
                next({ path: '/login' });
            }
        } else {
            const Title = getConfig().Title;
